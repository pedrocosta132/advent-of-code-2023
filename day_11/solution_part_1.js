...
for (let i = 0; i < points.length; i++) {
  ...
  for (let j = i + 1; j < points.length; j++) {
    ...
    //verify if how many empty lines intercept the 2 points
    emptyLines.forEach((l) => {
      if ((l > x1 && l < x2) || (l > x2 && l < x1)) sum += 1;
    });
    //verify if how many empty columns intercept the 2 points
    emptyColumns.forEach((c) => {
      if ((c > y1 && c < y2) || (c > y2 && c < y1)) sum += 1;
    });
